...
use crate::domain::value_objects::git_url::{GitUrl, GitUrlError};
use git2::{Direction, Repository as Git2Repository};
use thiserror::Error;
...
impl<'repo> GitRemoteManager<'repo> {
    ...
    pub fn set_remote_push_url(&self, name: &str, url: &GitUrl) -> Result<(), GitRemoteError> {
        ...
        // Update the push URL
        self.repo
            .remote_set_pushurl(name, Some(&url_str))
            .map_err(|e| GitRemoteError::UpdateRemoteUrlFailed(e.to_string()))?;
    ...
    pub fn add_or_update_remote(
        &self,
        name: &str,
        url: &GitUrl,
    ) -> Result<RemoteInfo, GitRemoteError> {
        if self.remote_exists(name) {
    ...
    pub fn setup_remotes(
        &self,
        remotes: &[DomainRemote],
    ) -> Result<Vec<RemoteInfo>, GitRemoteError> {
    ...
    fn validate_remote_name(&self, name: &str) -> Result<(), GitRemoteError> {
        ...
        if name
            .chars()
            .any(|c| invalid_chars.contains(&c) || c.is_ascii_control())
        {
            return Err(GitRemoteError::InvalidRemoteName(format!(
...
mod tests {
    ...
    fn test_add_duplicate_remote() {
        ...
        assert!(matches!(
            result.unwrap_err(),
            GitRemoteError::RemoteAlreadyExists(_)
        ));
    }
    ...
    fn test_utils_normalize_remote_name() {
        ...
        assert_eq!(
            utils::normalize_remote_name("remote with spaces"),
            "remote_with_spaces"
        );
        assert_eq!(utils::normalize_remote_name(".hidden"), "_hidden");
        assert_eq!(
            utils::normalize_remote_name("remote:with:colons"),
            "remote_with_colons"
        );
    }
