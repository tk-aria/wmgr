...
impl FilePath {
    ...
    fn check_path_traversal(path: &Path) -> Result<(), FilePathError> {
        for component in path.components() {
            match component {
                Component::ParentDir => {
                    return Err(FilePathError::PathTraversal(path.display().to_string()));
                }
                Component::CurDir => {
        ...
        for component in path_obj.components() {
            if let Component::Normal(os_str) = component {
                if let Some(name) = os_str.to_str() {
                    ...
                    {
                        return Err(FilePathError::HiddenFileBlocked(format!(
                            "System hidden file not allowed: {}",
                            name
                        )));
                    }
                }
...
mod tests {
    ...
    fn test_invalid_paths() {
        let invalid_cases = [
            ...
            (
                "../../../etc/passwd",
                FilePathError::PathTraversal("../../../etc/passwd".to_string()),
            ),
            (
                "dir/../../../file",
                FilePathError::PathTraversal("dir/../../../file".to_string()),
            ),
        ];
    ...
    fn test_dangerous_pattern_detection() {
        ...
        for path in dangerous_paths {
            let result = FilePath::new(path);
            assert!(
                matches!(result, Err(FilePathError::DangerousPattern(_))),
                "Should detect dangerous pattern in: {}",
                path
            );
        }
    }
    ...
    fn test_system_path_blocking() {
        ...
        for path in system_paths {
            let result = FilePath::new(path);
            assert!(
                matches!(result, Err(FilePathError::SystemPathBlocked(_))),
    ...
    fn test_hidden_file_handling() {
        ...
        for path in blocked_hidden {
            let result = FilePath::new_relative(path);
            assert!(
                matches!(result, Err(FilePathError::HiddenFileBlocked(_))),
                "Should block hidden file: {}",
                path
            );
        }
    }
    ...
    fn test_multiple_consecutive_characters() {
        let dangerous_patterns = ["file...", "path///to///file", "path\\\\\\to\\\\\\file"];

        for path in dangerous_patterns {
            let result = FilePath::new(path);
            assert!(
                matches!(result, Err(FilePathError::DangerousPattern(_))),
                "Should detect multiple consecutive characters in: {}",
                path
            );
        }
    }
    ...
    fn test_unicode_attack_patterns() {
        ...
        for path in unicode_attacks {
            ...
            assert!(
                matches!(result, Err(FilePathError::DangerousPattern(_))),
                "Should detect Unicode attack in: {:?}",
                path
            );
        }
    }
