...
impl BranchName {
    ...
    pub fn is_release_branch(&self) -> bool {
        let release_patterns = [r"^release/.*", r"^releases/.*", r"^rel/.*", r"^v\d+\.\d+.*"];

        release_patterns
            .iter()
            .any(|pattern| Regex::new(pattern).unwrap().is_match(&self.name))
    }

    /// フィーチャーブランチかどうかを判定
    pub fn is_feature_branch(&self) -> bool {
        let feature_patterns = [r"^feature/.*", r"^feat/.*", r"^features/.*"];
    ...
    pub fn is_hotfix_branch(&self) -> bool {
        let hotfix_patterns = [r"^hotfix/.*", r"^hotfixes/.*", r"^fix/.*"];
...
mod tests {
    ...
    fn test_invalid_branch_names() {
        let invalid_cases = [
            ...
            (
                "-branch",
                BranchNameError::StartsWithHyphen("-branch".to_string()),
            ),
            (
                "branch.lock",
                BranchNameError::EndsWithLock("branch.lock".to_string()),
            ),
            (
                "branch..name",
                BranchNameError::ConsecutiveDots("branch..name".to_string()),
            ),
            ("HEAD", BranchNameError::Reserved("HEAD".to_string())),
            (
                "branch name",
                BranchNameError::InvalidCharacter(" ".to_string()),
            ),
            (
                "branch:name",
                BranchNameError::InvalidCharacter(":".to_string()),
