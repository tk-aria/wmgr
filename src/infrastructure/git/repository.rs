...
use git2::{
    build::CheckoutBuilder, BranchType, Cred, CredentialType, FetchOptions, Oid, RemoteCallbacks,
    Repository as Git2Repository, ResetType,
};
...
impl GitRepository {
    ...
    pub fn checkout(&self, target: &str) -> Result<(), GitRepositoryError> {
        ...
        let reference = self
            ...
            .or_else(|_| {
                self.repo
                    .find_reference(&format!("refs/remotes/origin/{}", target))
            })
    ...
    pub fn create_branch(
        ...
        let start_commit = if let Some(start) = start_point {
            ...
            let reference = self
                ...
                .or_else(|_| {
                    self.repo
                        .find_reference(&format!("refs/remotes/origin/{}", start))
                })
        ...
        let _branch = self
            .repo
            .branch(branch_name.as_str(), &start_commit, false)?;
    ...
    pub fn reset(&self, target: &str, reset_type: ResetMode) -> Result<(), GitRepositoryError> {
        ...
        self.repo
            .reset(target_commit.as_object(), git2_reset_type, None)?;
    ...
    pub fn list_branches(
        &self,
        branch_type: Option<GitBranchType>,
    ...
    fn calculate_ahead_behind(
        &self,
        branch_name: &str,
    ) -> Result<(usize, usize), GitRepositoryError> {
...
mod tests {
    ...
    fn test_repository_open_nonexistent() {
        ...
        assert!(matches!(
            result.unwrap_err(),
            GitRepositoryError::RepositoryNotFound(_)
        ));
    }
