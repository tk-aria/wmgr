...
impl ParallelResult {
    ...
    pub fn add_result(
        &mut self,
        task_id: String,
        result: Result<ExecutionResult, CommandExecutorError>,
    ) {
        match &result {
...
impl CommandExecutor {
    ...
    pub async fn execute(
        ...
        let child = cmd.spawn().map_err(|e| {
            CommandExecutorError::SpawnFailed(format!("Failed to spawn '{}': {}", command, e))
        })?;
    ...
    fn parse_command(
        ...
        if use_shell {
            ...
            if parts.is_empty() {
                return Err(CommandExecutorError::InvalidCommand(
                    "Command has no parts".to_string(),
                ));
            }
    ...
    async fn wait_for_completion(
        ...
        let exit_status = child.wait().await.map_err(|e| {
            CommandExecutorError::TerminationFailed(format!("Failed to wait for process: {}", e))
        })?;
...
mod tests {
    ...
    async fn test_command_with_environment_variables() {
        let config = ExecutionConfig::new().with_environment_variable("TEST_VAR", "test_value");
    ...
    async fn test_parallel_execution() {
        ...
        for (task_id, task_result) in &result.task_results {
            ...
            assert!(exec_result
                .stdout
                .contains(&format!("Task {}", task_id.chars().last().unwrap())));
        }
    }
    ...
    fn test_command_parsing() {
        let (program, args) =
            CommandExecutor::parse_command("git status --porcelain", false).unwrap();
