...
impl ConfigStore {
    ...
    pub fn with_config(backup_config: BackupConfig, validation_config: ValidationConfig) -> Self {
        Self {
            backup_config,
            validation_config,
        }
    ...
    pub fn write_config<T, P>(&self, config_path: P, config: &T) -> Result<(), ConfigStoreError>
    where
        T: Serialize,
        P: AsRef<Path>,
    {
    ...
    pub fn delete_config<P: AsRef<Path>>(&self, config_path: P) -> Result<(), ConfigStoreError> {
        let config_path = config_path.as_ref();
        ...
        fs::remove_file(config_path).map_err(|e| ConfigStoreError::WriteFailed(e.to_string()))?;
    ...
    pub fn list_backups<P: AsRef<Path>>(
        ...
        let base_name = config_path.file_name().ok_or_else(|| {
            ConfigStoreError::InvalidConfigPath(config_path.display().to_string())
        })?;
...
mod tests {
    ...
    fn test_read_nonexistent_config() {
        ...
        assert!(matches!(
            result.unwrap_err(),
            ConfigStoreError::ConfigFileNotFound(_)
        ));
    }
    ...
    fn test_backup_functionality() {
        ...
        store
            .write_workspace_config(&config_path, &modified_config)
            .unwrap();
