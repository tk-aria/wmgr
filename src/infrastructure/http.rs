...
impl HttpDownloader {
    ...
    pub fn download_and_extract(&self, url: &str, dest_path: &Path) -> TsrcResult<()> {
        info!("Downloading from URL: {}", url);
        
        // Create parent directory if it doesn't exist
        if let Some(parent) = dest_path.parent() {
            fs::create_dir_all(parent).map_err(|e| {
                TsrcError::IoError(format!("Failed to create parent directory: {}", e))
            })?;
        }

        // Download the file
        let response = self.client.get(url).send().map_err(|e| {
            TsrcError::network_error_with_source(
        ...
        if needs_extraction {
            ...
            let temp_file = tempfile::NamedTempFile::new().map_err(|e| {
                TsrcError::IoError(format!("Failed to create temp file: {}", e))
            })?;

            let mut temp_file_write = temp_file.reopen().map_err(|e| {
                TsrcError::IoError(format!("Failed to open temp file for writing: {}", e))
            })?;
            ...
            temp_file_write.write_all(&content).map_err(|e| {
                TsrcError::IoError(format!("Failed to write to temp file: {}", e))
            })?;
            
            temp_file_write.flush().map_err(|e| {
                TsrcError::IoError(format!("Failed to flush temp file: {}", e))
            })?;
            ...
            let mut file = File::create(dest_path).map_err(|e| {
                TsrcError::IoError(format!("Failed to create file {}: {}", dest_path.display(), e))
            })?;

            file.write_all(&content).map_err(|e| {
                TsrcError::IoError(format!("Failed to write file {}: {}", dest_path.display(), e))
            })?;
    ...
    fn extract_archive(&self, archive_path: &Path, url_path: &str, dest_path: &Path) -> TsrcResult<()> {
        debug!("Extracting archive: {} to {}", archive_path.display(), dest_path.display());

        // Create destination directory
        fs::create_dir_all(dest_path).map_err(|e| {
            TsrcError::IoError(format!("Failed to create destination directory: {}", e))
        })?;
        ...
        if url_path.ends_with(".zip") {
            ...
            return Err(TsrcError::UnsupportedOperation(
    ...
    /// Extract ZIP archive
    fn extract_zip(&self, archive_path: &Path, dest_path: &Path) -> TsrcResult<()> {
        let file = File::open(archive_path).map_err(|e| {
            TsrcError::IoError(format!("Failed to open zip file: {}", e))
        })?;

        let mut archive = ZipArchive::new(file).map_err(|e| {
            TsrcError::IoError(format!("Failed to read zip archive: {}", e))
        })?;

        for i in 0..archive.len() {
            let mut file = archive.by_index(i).map_err(|e| {
                TsrcError::IoError(format!("Failed to read zip entry: {}", e))
            })?;
            ...
            if file.name().ends_with('/') {
                fs::create_dir_all(&outpath).map_err(|e| {
                    TsrcError::IoError(format!("Failed to create directory: {}", e))
                })?;
            } else {
                ...
                let mut outfile = File::create(&outpath).map_err(|e| {
                    TsrcError::IoError(format!("Failed to create file: {}", e))
                })?;
                io::copy(&mut file, &mut outfile).map_err(|e| {
                    TsrcError::IoError(format!("Failed to extract file: {}", e))
                })?;
            }
    ...
    fn extract_tar_gz(&self, archive_path: &Path, dest_path: &Path) -> TsrcResult<()> {
        let tar_gz = File::open(archive_path).map_err(|e| {
            TsrcError::IoError(format!("Failed to open tar.gz file: {}", e))
        })?;
        ...
        archive.unpack(dest_path).map_err(|e| {
            TsrcError::IoError(format!("Failed to extract tar.gz archive: {}", e))
        })?;
    ...
    /// Extract tar archive
    fn extract_tar(&self, archive_path: &Path, dest_path: &Path) -> TsrcResult<()> {
        let tar = File::open(archive_path).map_err(|e| {
            TsrcError::IoError(format!("Failed to open tar file: {}", e))
        })?;

        let mut archive = Archive::new(tar);

        archive.unpack(dest_path).map_err(|e| {
            TsrcError::IoError(format!("Failed to extract tar archive: {}", e))
        })?;
    ...
    pub fn download_file(&self, url: &str, dest_path: &Path) -> TsrcResult<()> {
