...
impl SecurityService {
    ...
    pub async fn audit_dependencies(
        &self,
        project_path: &Path,
    ) -> Result<AuditResult, SecurityError> {
    ...
    pub async fn audit_dependencies_silent(
        &self,
        project_path: &Path,
    ...
    fn check_cargo_audit_available(&self) -> Result<(), SecurityError> {
        ...
        if !output.status.success() {
            return Err(SecurityError::CommandExecutionFailed(
                "cargo-audit is not installed. Run 'cargo install cargo-audit' to install."
                    .to_string(),
            ));
        }
    ...
    /// JSONから脆弱性情報を抽出
    fn extract_vulnerability_from_json(
        &self,
        json: &serde_json::Value,
    ) -> Result<Option<Vulnerability>, SecurityError> {
...
mod tests {
    ...
    fn test_vulnerability_severity_parsing() {
        ...
        assert_eq!(
            service.parse_severity("medium"),
            VulnerabilitySeverity::Medium
        );
        assert_eq!(service.parse_severity("high"), VulnerabilitySeverity::High);
        assert_eq!(
            service.parse_severity("critical"),
            VulnerabilitySeverity::Critical
        );
        assert_eq!(
            service.parse_severity("unknown"),
            VulnerabilitySeverity::Medium
        );
    }
