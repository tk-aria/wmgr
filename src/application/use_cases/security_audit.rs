...
use thiserror::Error;
use crate::application::services::security_service::{SecurityService, AuditResult, SecurityError};
...
impl SecurityAuditUseCase {
    ...
    /// セキュリティ監査を実行
    pub async fn execute(&self, workspace: &Workspace) -> Result<WorkspaceAuditResult, SecurityAuditError> {
        // 1. ワークスペースの初期化チェック
        ...
        let rust_repos = self.filter_rust_repositories(workspace, &target_repos).await?;
    ...
    /// 監査対象リポジトリの決定
    fn determine_target_repositories(&self, workspace: &Workspace) -> Result<Vec<ManifestRepo>, SecurityAuditError> {
    ...
    /// Rustプロジェクトをフィルタリング
    async fn filter_rust_repositories(&self, workspace: &Workspace, repos: &[ManifestRepo]) -> Result<Vec<ManifestRepo>, SecurityAuditError> {
    ...
    /// 順次監査実行
    async fn execute_sequential(&self, workspace: &Workspace, target_repos: &[ManifestRepo]) -> Result<WorkspaceAuditResult, SecurityAuditError> {
    ...
    /// 並列監査実行
    async fn execute_parallel(&self, workspace: &Workspace, target_repos: &[ManifestRepo]) -> Result<WorkspaceAuditResult, SecurityAuditError> {
        let mut result = WorkspaceAuditResult::new(true);
        
        if self.config.verbose {
            println!("Auditing dependencies in {} repositories in parallel", target_repos.len());
        }
            ...
            tokio::spawn(async move {
                let _permit = semaphore.acquire().await.map_err(|e| {
                    SecurityAuditError::SecurityServiceError(
                        SecurityError::CommandExecutionFailed(format!("Failed to acquire semaphore: {}", e))
                    )
                })?;
                ...
                Ok::<RepoAuditResult, SecurityAuditError>(use_case.audit_repository(&workspace, &repo).await)
            })
        }).collect();
    ...
    /// 単一リポジトリの監査
    async fn audit_repository(&self, workspace: &Workspace, repo: &ManifestRepo) -> RepoAuditResult {
        let repo_path = workspace.repo_path(&repo.dest);
...
mod tests {
    ...
    use crate::domain::entities::{workspace::WorkspaceConfig, manifest::Manifest};
    use std::fs;
    ...
    async fn test_workspace_not_initialized() {
        ...
        assert!(matches!(result.unwrap_err(), SecurityAuditError::WorkspaceNotInitialized(_)));
    }
}