...
impl ForeachCommandUseCase {
    ...
    pub async fn execute(
        ...
        let result = if self.config.parallel {
            self.execute_parallel(&target_repos, workspace, &env_vars)
                .await?
        } else {
            self.execute_sequential(&target_repos, workspace, &env_vars)
                .await?
        };
    ...
    fn determine_target_repositories(
        &self,
        workspace: &Workspace,
    ) -> Result<Vec<ManifestRepo>, ForeachCommandError> {
        let manifest = workspace
            .manifest
            .as_ref()
            .ok_or_else(|| ForeachCommandError::ProcessError("Manifest not loaded".to_string()))?;
    ...
    fn prepare_environment_variables(
        &self,
        workspace: &Workspace,
    ) -> Result<HashMap<String, String>, ForeachCommandError> {
    ...
    async fn execute_sequential(
        ...
        for repo in target_repos {
            ...
            let command_result = self
                .execute_command_in_repo(repo, workspace, env_vars)
                .await;
            ...
            match command_result {
                ...
                Err(e) => {
                    let failed_result =
                        CommandResult::new(repo.dest.clone()).with_failure(None, e.to_string(), 0);
    ...
    async fn execute_parallel(
        ...
        if self.config.verbose {
            println!(
                "Executing commands in {} repositories in parallel",
                target_repos.len()
            );
        }
            ...
            .map(|repo| {
                ...
                tokio::spawn(async move {
                    let _permit = semaphore.acquire().await.map_err(|e| {
                        ForeachCommandError::ParallelExecutionFailed(format!(
        ...
        for (i, join_result) in results.into_iter().enumerate() {
            match join_result {
                Ok(task_result) => match task_result {
                    ...
                    Err(e) => {
                        let repo_name = target_repos
                            .get(i)
                            .map(|r| r.dest.clone())
                            .unwrap_or_else(|| "unknown".to_string());
                        let failed_result =
                            CommandResult::new(repo_name).with_failure(None, e.to_string(), 0);
                        result.add_result(failed_result);
                ...
                Err(join_err) => {
                    let repo_name = target_repos
                        ...
                        .map(|r| r.dest.clone())
                        .unwrap_or_else(|| "unknown".to_string());
                    let failed_result = CommandResult::new(repo_name).with_failure(
                        None,
                    ...
                    if !self.config.continue_on_error {
                        return Err(ForeachCommandError::ParallelExecutionFailed(format!(
                            "Task join error: {}",
                            join_err
                        )));
                    }
    ...
    async fn execute_command_in_repo(
        ...
        // コマンド実行
        self.perform_command_execution(
            &self.config.command,
            &working_dir,
            &repo_env_vars,
            &repo.dest,
    ...
    async fn perform_command_execution(
        ...
        let result = CommandResult::new(repo_dest.to_string()).with_success(
            0,
...
mod tests {
    ...
    fn test_foreach_config_creation() {
        ...
        assert_eq!(
            config.environment_variables.get("TEST_VAR"),
    ...
    fn test_command_result_status() {
        let success_result = CommandResult::new("repo".to_string()).with_success(
            0,
            "output".to_string(),
            String::new(),
            100,
    ...
    fn test_foreach_result_counting() {
        ...
        let success_result = CommandResult::new("success".to_string()).with_success(
            0,
            "output".to_string(),
            String::new(),
            100,
        );
        let failed_result = CommandResult::new("failed".to_string()).with_failure(
            Some(1),
            "error".to_string(),
            200,
        );
        let skipped_result =
            CommandResult::new("skipped".to_string()).with_skip("No repo".to_string());
