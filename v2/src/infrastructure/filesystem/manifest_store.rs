...
use crate::domain::entities::manifest::{Manifest, ManifestRepo, FileCopy, FileSymlink};
use crate::application::services::manifest_service::{ManifestService, ManifestServiceError, ProcessedManifest};
...
impl ManifestStore {
    ...
    pub async fn read_manifest<P: AsRef<Path>>(
        ...
        let processed_manifest = self.manifest_service
    ...
    pub async fn write_manifest<P: AsRef<Path>>(
        ...
        let yaml_content = self.manifest_service
        ...
        for repo in &manifest.repos {
            if let Some(symlink_operations) = &repo.symlink {
                for symlink_op in symlink_operations {
                    let result = self.execute_symlink_operation(
                        symlink_op,
                        repo,
                        workspace_root,
                    ).await;
    ...
    pub async fn process_all_file_operations<P: AsRef<Path>>(
        ...
        let mut copy_results = self.process_copy_operations(manifest, workspace_root).await?;
        results.append(&mut copy_results);
        
        // Process symlink operations
        let mut symlink_results = self.process_symlink_operations(manifest, workspace_root).await?;
    ...
    pub async fn get_manifest_metadata<P: AsRef<Path>>(
        ...
        if manifest_path.exists() {
            ...
            metadata.last_modified = fs_metadata.modified()
                .map_err(|e| ManifestStoreError::ReadFailed(e.to_string()))?;
            ...
            if let Ok(processed_manifest) = self.read_manifest(manifest_path).await {
                ...
                metadata.group_count = processed_manifest.manifest.groups
                    .as_ref()
                    .map(|g| g.len())
                    .unwrap_or(0);
            }
    ...
    fn validate_copy_paths(
        ...
        if source_path.to_string_lossy().contains("..") {
            return Err(ManifestStoreError::PathValidationFailed(
                format!("Source path contains path traversal: {}", source_path.display())
            ));
        }
        
        if dest_path.to_string_lossy().contains("..") {
            return Err(ManifestStoreError::PathValidationFailed(
                format!("Destination path contains path traversal: {}", dest_path.display())
            ));
        }
    ...
    fn validate_symlink_paths(
        ...
        if target_path.is_absolute() && target_path.to_string_lossy().contains("..") {
            return Err(ManifestStoreError::PathValidationFailed(
                format!("Target path contains path traversal: {}", target_path.display())
            ));
        }
    ...
    async fn cleanup_old_backups<P: AsRef<Path>>(&self, file_path: P) -> Result<(), ManifestStoreError> {
        let file_path = file_path.as_ref();
        let parent = file_path.parent().unwrap_or(Path::new("."));
        let base_name = file_path.file_name()
            .ok_or_else(|| ManifestStoreError::InvalidManifestPath(
        ...
        if backups.len() > self.options.file_operation_config.max_backups {
            for backup in backups.iter().skip(self.options.file_operation_config.max_backups) {
...
mod tests {
    ...
    async fn create_test_manifest() -> Manifest {
        ...
        groups.insert(
            "core".to_string(),
            Group::new(vec!["repo1".to_string()])
                .with_description("Core repositories"),
        );
    ...
    async fn test_write_and_read_manifest() {
        ...
        // Write manifest
        let result = store.write_manifest(&manifest_path, &original_manifest).await;
        assert!(result.is_ok());
    ...
    async fn test_read_nonexistent_manifest() {
        ...
        assert!(matches!(result.unwrap_err(), ManifestStoreError::ManifestFileNotFound(_)));
    }
    ...
    async fn test_manifest_metadata() {
        ...
        // Write manifest
        store.write_manifest(&manifest_path, &manifest).await.unwrap();
    ...
    async fn test_file_operations_processing() {
        ...
        async_fs::write(repo1_dir.join("config.yml"), "test config").await.unwrap();
        ...
        // Process copy operations
        let copy_results = store.process_copy_operations(&manifest, workspace_root).await.unwrap();
        ...
        // Process symlink operations
        let symlink_results = store.process_symlink_operations(&manifest, workspace_root).await.unwrap();
    ...
    async fn test_process_all_file_operations() {
        ...
        let results = store.process_all_file_operations(&manifest, workspace_root).await.unwrap();
    ...
    async fn test_filter_manifest_by_groups() {
        ...
        let filtered = store.filter_manifest_by_groups(&manifest, &["core".to_string()]).unwrap();
    ...
    async fn test_backup_functionality() {
        ...
        store.write_manifest(&manifest_path, &modified_manifest).await.unwrap();
