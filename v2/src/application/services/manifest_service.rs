...
use crate::domain::entities::manifest::{Manifest, ManifestRepo, Group};
...
impl ManifestService {
    ...
    pub async fn parse_from_file(&mut self, path: &Path) -> Result<ProcessedManifest, ManifestServiceError> {
        let content = tokio::fs::read_to_string(path).await?;
        self.parse_from_string(&content, Some(path.to_path_buf())).await
    }
    
    /// URL からマニフェストを読み込み・解析
    pub async fn parse_from_url(&mut self, url: &str) -> Result<ProcessedManifest, ManifestServiceError> {
        // キャッシュをチェック
    ...
    pub async fn parse_from_string(
        ...
        // Deep manifest処理
        let processed = self.process_deep_manifest(extended_manifest, base_path, 0, &mut Vec::new()).await?;
        ...
        for repo in &manifest.repos {
            if !dest_set.insert(&repo.dest) {
                return Err(ManifestServiceError::ValidationError(
                    format!("Duplicate destination path: {}", repo.dest)
                ));
            }
            ...
            for (group_name, group) in groups {
                for repo_dest in &group.repos {
                    if !manifest.repos.iter().any(|r| &r.dest == repo_dest) {
                        return Err(ManifestServiceError::ValidationError(
                            format!("Group '{}' references non-existent repository: {}", group_name, repo_dest)
                        ));
                    }
                }
            }
    ...
    pub fn filter_by_groups(&self, manifest: &Manifest, group_names: &[String]) -> Result<Manifest, ManifestServiceError> {
        ...
        if let Some(groups) = &manifest.groups {
            for group_name in group_names {
                if let Some(group) = groups.get(group_name) {
                    ...
                    let filtered_group_repos: Vec<String> = group.repos
                        .iter()
    ...
    /// 特定のグループの詳細情報を取得
    pub fn get_group_info<'a>(&self, manifest: &'a Manifest, group_name: &str) -> Option<(&'a Group, Vec<&'a ManifestRepo>)> {
        if let Some(groups) = &manifest.groups {
            if let Some(group) = groups.get(group_name) {
    ...
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<ProcessedManifest, ManifestServiceError>> + 'a>> {
        Box::pin(async move {
        // 深度制限チェック
        ...
        if let Some(includes) = &extended_manifest.includes {
            for include in includes {
                ...
                if self.options.detect_circular_dependencies && visited.contains(&include_url) {
                    return Err(ManifestServiceError::CircularDependency(
                        format!("Circular dependency detected: {} -> {}", visited.join(" -> "), include_url)
                    ));
                }
                ...
                let included_extended: ExtendedManifest = serde_yaml::from_str(&included_content)?;
                
                let included_processed = self.process_deep_manifest(
                    included_extended,
                    None,
                    depth + 1,
                    visited,
                ...
                all_includes.push(IncludeInfo {
                    url: include_url,
                    revision: include.revision.clone().unwrap_or_else(|| "HEAD".to_string()),
                    repo_count,
                });
        ...
        if let Some(deprecated) = &future_config.deprecated {
            for dep in deprecated {
                let warning = if let Some(removal_version) = &dep.removal_version {
                    format!("DEPRECATED: {} (will be removed in {}): {}", dep.feature, removal_version, dep.message)
                } else {
    ...
    /// インクルードURLを解決
    fn resolve_include_url(&self, url: &str, base_path: &Option<PathBuf>) -> Result<String, ManifestServiceError> {
        // 既に絶対URLの場合はそのまま返す
    ...
    fn merge_manifests(
        ...
        let mut dest_set: std::collections::HashSet<String> = base.repos
            .iter()
            .map(|r| r.dest.clone())
            .collect();
...
mod tests {
    use super::*;
    use crate::domain::entities::manifest::{ManifestRepo, Group};
...
"#;
        ...
        assert!(matches!(result.unwrap_err(), ManifestServiceError::ValidationError(_)));
    }
    ...
    async fn test_filter_by_groups() {
        ...
        groups.insert(
            "group2".to_string(),
            Group::new(vec!["repo3".to_string()]),
        );
        
        let manifest = Manifest::new(repos).with_groups(groups);
        let service = ManifestService::default();
        
        let filtered = service.filter_by_groups(&manifest, &["group1".to_string()]).unwrap();
    ...
    async fn test_group_not_found() {
        ...
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), ManifestServiceError::GroupNotFound(_)));
    }
...
"#;
        ...
        tokio::fs::write(&manifest_file, yaml_content).await.unwrap();
...
"#;
    ...
    fn test_merge_manifests() {
        ...
        let merged = service.merge_manifests(base_manifest, include_manifest, 1).unwrap();
