...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (task_id, result) in &parallel_result.task_results {
        match result {
            ...
            Ok(exec_result) => {
                println!("  {} failed with exit code: {}", task_id, exec_result.exit_code);
            }
            Err(e) => {
    ...
    for repo in &repo_dirs {
        ...
        let task = ExecutionTask::new(repo.to_string(), "git status --porcelain")
            .with_config(config);
        tasks.push(task);
    }
    ...
    for (repo_name, result) in &repo_result.task_results {
        match result {
            ...
            Ok(exec_result) => {
                println!("  {}: Git status failed (exit code: {})", repo_name, exec_result.exit_code);
            }
            Err(e) => {
