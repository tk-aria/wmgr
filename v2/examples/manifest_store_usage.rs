use tsrc::infrastructure::filesystem::manifest_store::{ManifestStore, ManifestProcessingOptions, FileOperationConfig};
use tsrc::domain::entities::manifest::{Manifest, ManifestRepo, FileCopy, FileSymlink, Group};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    groups.insert(
        "core".to_string(),
        Group::new(vec!["core".to_string()])
            .with_description("Core repositories"),
    );
    groups.insert(
        "tools".to_string(),
        Group::new(vec!["utils".to_string()])
            .with_description("Utility repositories"),
    );
    ...
    println!("   Read {} repositories", processed_manifest.manifest.repos.len());
    println!("   Groups: {:?}", store.list_manifest_groups(&processed_manifest.manifest));
    ...
    println!("   Filtered manifest has {} repositories", core_manifest.repos.len());
    ...
    tokio::fs::write(core_dir.join("config/default.yml"), "# Default config\nkey: value").await?;
    tokio::fs::write(core_dir.join("scripts/build.sh"), "#!/bin/bash\necho 'Building...'").await?;
    ...
    tokio::fs::write(utils_dir.join("bin/utils"), "#!/bin/bash\necho 'Utils tool'").await?;
    
    // 8. Process file copy operations
    println!("8. Processing file copy operations...");
    let copy_results = store.process_copy_operations(&manifest, workspace_root).await?;
    ...
    let symlink_results = store.process_symlink_operations(&manifest, workspace_root).await?;
    ...
    let all_results = store.process_all_file_operations(&manifest, workspace_root).await?;
    ...
    store.write_manifest(&manifest_path, &updated_manifest).await?;
