...
impl GitUrlHelper {
    ...
    pub fn create_test_urls(base_org: &str, base_repo: &str) -> Vec<GitUrl> {
        vec![
            GitUrl::new(&format!("https://github.com/{}/{}.git", base_org, base_repo)).unwrap(),
            GitUrl::new(&format!("git@github.com:{}/{}.git", base_org, base_repo)).unwrap(),
            GitUrl::new(&format!("https://gitlab.com/{}/{}.git", base_org, base_repo)).unwrap(),
...
mod tests {
    use super::*;
    use tempfile::TempDir;
    use crate::common::test_fixtures::{WorkspaceFixture, ManifestFixture};
    ...
    async fn test_async_helper_timeout() {
        let result = AsyncHelper::with_timeout(
            async { sleep(Duration::from_millis(10)).await; 42 },
        ...
        let result = AsyncHelper::with_timeout(
            async { sleep(Duration::from_millis(100)).await; 42 },
