...
use tsrc::{
    infrastructure::git::{
        repository::{GitRepository, CloneConfig, FetchConfig, ResetMode, GitBranchType},
        remote::{GitRemoteManager, RemoteInfo},
    },
    domain::{
        entities::repository::{Repository, Remote},
        value_objects::{
            git_url::GitUrl,
...
fn init_test_repository(dir: &PathBuf) -> std::io::Result<()> {
    ...
    std::fs::write(dir.join(".git").join("refs").join("heads").join("main"), "dummy-commit-hash\n")?;
    
    // テスト用ファイルを追加
    std::fs::write(dir.join("README.md"), "# Test Repository\n")?;
    std::fs::write(dir.join("src").join("main.rs"), "fn main() { println!(\"Hello, world!\"); }\n")?;
...
async fn test_git_repository_initialization() {
    ...
    // 3. リポジトリが有効であることを確認
    assert!(git_repo.is_valid(), "Repository should be valid after initialization");
    
    // 4. 現在のブランチを確認
    let current_branch = git_repo.current_branch();
    assert!(current_branch.is_ok(), "Should be able to get current branch");
}
...
async fn test_git_repository_opening() {
    ...
    assert!(invalid_result.is_err(), "Should fail to open nonexistent repository");
}
...
async fn test_git_remote_manager_operations() {
    ...
    assert!(remote_manager.is_ok(), "Should be able to create remote manager");
    ...
    assert_eq!(remote_list[0].name, "origin", "Remote name should be 'origin'");
    ...
    assert_eq!(info.fetch_url.as_str(), "https://github.com/example/repo.git");
    ...
    assert!(upstream_result.is_ok(), "Should be able to add upstream remote");
...
async fn test_domain_repository_entity_integration() {
    ...
    assert_eq!(origin_remote.unwrap().url, "https://github.com/example/repo.git");
    ...
    assert!(nonexistent_remote.is_none(), "Should not find nonexistent remote");
...
async fn test_git_url_and_repository_integration() {
    ...
    // 2. URLの等価性を確認
    assert!(https_url.is_same_repo(&ssh_url), "HTTPS and SSH URLs should point to same repo");
    
    // 3. URL変換をテスト
    assert_eq!(https_url.to_ssh_url(), "git@github.com:example/repo.git");
    assert_eq!(ssh_url.to_https_url(), "https://github.com/example/repo.git");
...
async fn test_git_operations_error_handling() {
    ...
    assert!(file_result.is_err(), "Should fail when path is an existing file");
    ...
    assert!(invalid_url_result.is_err(), "Should fail to parse invalid URL");
    ...
    assert!(duplicate_result.is_err(), "Should fail to add duplicate remote");
}
...
async fn test_comprehensive_git_workflow() {
    ...
    assert!(branches.len() >= 2, "Should have at least main and feature branch");
    ...
    let domain_remotes = vec![
        Remote::new("origin", origin_url.as_str()),
    ];
    let repository = Repository::new(&repo_path.file_name().unwrap().to_str().unwrap(), domain_remotes);
