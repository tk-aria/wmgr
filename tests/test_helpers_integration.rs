...
use common::{
    ...
    mock_services::{MockCommandExecutor, MockFileSystem, MockGitRepository, MockManifestService},
    test_fixtures::{
        FileSystemFixture, ManifestFixture, RepositoryFixture, ValueObjectFixture, WorkspaceFixture,
    },
    test_helpers::{
        AsyncHelper, EnvironmentHelper, FileSystemHelper, GitUrlHelper, WorkspaceHelper,
    },
};
...
async fn test_async_helpers() {
    // Test timeout
    let result = AsyncHelper::with_timeout(async { 42 }, Duration::from_millis(100)).await;
    assert!(result.is_ok());
...
async fn test_mock_services() {
    ...
    let result = mock_service
        .parse_from_file(std::path::Path::new("/test/manifest.yml"))
        .await;
    assert!(result.is_ok());
