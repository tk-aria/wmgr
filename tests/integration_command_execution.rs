...
async fn test_command_executor_basic_operations() {
    ...
    let result = executor
        .execute("echo 'Hello, World!'", &simple_config)
        .await;
    ...
    assert!(
        execution_result.stdout.contains("Hello, World!"),
    ...
    assert!(
        fail_result.is_ok(),
        "Command execution should return result even for failed commands"
    );

    let fail_execution = fail_result.unwrap();
    assert_ne!(
        fail_execution.exit_code, 0,
    ...
    #[cfg(unix)]
    {
        ...
        assert!(
            env_result.is_ok(),
            "Environment variable command should succeed"
        );

        let env_execution = env_result.unwrap();
        assert!(
            env_execution.stdout.contains("test_value"),
...
async fn test_parallel_command_execution() {
    ...
    for (task_id, execution_result) in results {
        ...
        assert!(
            execution_result.stdout.contains("Task"),
            "Output should contain task information"
        );
    }
}
...
async fn test_foreach_command_use_case() {
    ...
    // 3. 実行結果を確認
    assert!(
        foreach_result.is_success(),
        "Foreach should complete successfully"
    );
    assert_eq!(
        foreach_result.total_count(),
        2,
        "Should execute command in 2 repositories"
    );
...
async fn test_foreach_command_with_environment_variables() {
    ...
    assert!(
        result.is_ok(),
        "Foreach command with environment variables should succeed"
    );
    let foreach_result = result.unwrap();
...
async fn test_foreach_command_error_handling() {
    ...
    assert!(
        result.is_ok(),
        "Foreach command should return result even with failures"
    );
    let foreach_result = result.unwrap();
    ...
    assert!(
        !foreach_result.is_success(),
        "Should not be successful due to command failures"
    );
...
async fn test_foreach_command_parallel_execution() {
    ...
    assert_eq!(
        foreach_result.total_count(),
        2,
        "Should execute in 2 repositories"
    );
}
...
async fn test_comprehensive_command_workflow() {
    ...
    for command in commands {
        ...
        assert!(
            ...
            "Should execute command in repositories"
        );
        ...
        for command_result in &result.results {
            if command_result.status == CommandStatus::Success {
                assert!(
                    command_result.stdout.len() > 0 || command_result.stderr.len() > 0,
    ...
    // 2. 環境変数を使用した複雑なコマンド
    let env_config =
        ForeachCommandConfig::new("echo \"Repository: $TSRC_REPO_DEST, URL: $TSRC_REPO_URL\"")
    ...
    assert!(
        ...
        "Workspace should remain initialized"
    );
