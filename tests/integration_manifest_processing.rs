...
use tsrc::{
    application::services::manifest_service::{ManifestService, ManifestProcessingOptions, ProcessedManifest},
    domain::entities::manifest::{Manifest, ManifestRepo, Group},
    infrastructure::filesystem::manifest_store::{ManifestStore, ManifestProcessingOptions as StoreProcessingOptions},
};
...
async fn test_manifest_store_read_and_write() {
    ...
    // 1. テスト用マニフェストファイルを作成
    let original_manifest_path = create_complex_manifest_file(temp_dir.path(), "original.yml").unwrap();
    
    // 2. マニフェストを読み込み
    let loaded_processed = manifest_store.read_manifest(&original_manifest_path).await.unwrap();
    let loaded_manifest = &loaded_processed.manifest;
    
    // 3. 読み込み結果を確認
    assert_eq!(loaded_manifest.repos.len(), 4, "Should load 4 repositories");
    assert!(loaded_manifest.groups.is_some(), "Should have groups defined");
    ...
    assert!(groups.contains_key("frontend"), "Should have frontend group");
    
    // 4. マニフェストを別の場所に書き込み
    let output_path = temp_dir.path().join("output.yml");
    manifest_store.write_manifest(&output_path, loaded_manifest).await.unwrap();
    ...
    assert_eq!(loaded_manifest.groups.is_some(), reloaded_manifest.groups.is_some());
}
...
async fn test_manifest_service_processing() {
    ...
    // 2. マニフェストをファイルから解析
    let processed_manifest = manifest_service.parse_from_file(&manifest_path).await.unwrap();
    ...
    // 4. グループ情報を取得
    let (backend_group, backend_repos) = manifest_service.get_group_info(&processed_manifest.manifest, "backend").unwrap();
    assert_eq!(backend_repos.len(), 1);
    assert_eq!(backend_repos[0].dest, "backend/api");
    
    let (core_group, core_repos) = manifest_service.get_group_info(&processed_manifest.manifest, "core").unwrap();
    ...
    // 5. 存在しないグループのテスト
    let invalid_group = manifest_service.get_group_info(&processed_manifest.manifest, "nonexistent");
...
async fn test_manifest_filtering_by_groups() {
    ...
    let filtered_manifest = manifest_store.filter_manifest_by_groups(
    ...
    let multi_filtered = manifest_store.filter_manifest_by_groups(
        &processed_manifest.manifest,
        &multiple_groups
    ).unwrap();
    
    assert_eq!(multi_filtered.repos.len(), 2);
    let dest_names: Vec<&str> = multi_filtered.repos.iter().map(|r| r.dest.as_str()).collect();
    ...
    let empty_filtered = manifest_store.filter_manifest_by_groups(
        &processed_manifest.manifest,
        &invalid_groups
    ).unwrap();
    
    assert_eq!(empty_filtered.repos.len(), 0, "Should return empty manifest for nonexistent groups");
}
...
async fn test_manifest_validation_and_error_handling() {
    ...
    assert!(missing_result.is_err(), "Should fail to read nonexistent file");
    ...
    assert_eq!(empty_processed.manifest.repos.len(), 0, "Should have no repositories");
}
...
async fn test_manifest_processing_with_options() {
    ...
    assert!(result.is_ok(), "Should read manifest with validation options");
...
async fn test_end_to_end_manifest_workflow() {
    ...
    // 2. マニフェストサービスで処理
    let processed = manifest_service.parse_from_file(&source_path).await.unwrap();
    ...
    manifest_store.write_manifest(&output_path, &filtered_manifest).await.unwrap();
    
    // 5. 書き込んだファイルを再読み込みして検証
    let reloaded_processed = manifest_store.read_manifest(&output_path).await.unwrap();
    assert_eq!(reloaded_processed.manifest.repos.len(), 2, "Should have 2 core repositories");
    
    // 6. リポジトリ情報を確認
    let repo_dests: Vec<&str> = reloaded_processed.manifest.repos.iter().map(|r| r.dest.as_str()).collect();
    assert!(repo_dests.contains(&"backend/api"));
    assert!(repo_dests.contains(&"frontend/web"));
    
    // 7. マニフェストサービスでの最終検証
    let final_processed = manifest_service.parse_from_file(&output_path).await.unwrap();
