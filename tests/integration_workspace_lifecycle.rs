...
use tsrc::{
    application::use_cases::{
        ...
        sync_repositories::{SyncRepositoriesConfig, SyncRepositoriesUseCase},
    },
    domain::{
        entities::{
        ...
        value_objects::{file_path::FilePath, git_url::GitUrl},
    },
};
...
async fn test_workspace_initialization_workflow() {
    ...
    assert_eq!(
        workspace.config.manifest_url,
        "https://github.com/example/manifest"
    );
...
async fn test_repository_synchronization_workflow() {
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                e.to_string().contains("Repository clone failed")
    ...
    assert!(
        ...
        "Workspace should be in initialized state"
    );
}
...
async fn test_status_check_workflow() {
    ...
    // ステータス結果を確認
    assert_eq!(
        status_result.total_count(),
        2,
        "Should check 2 repositories"
    );
    // リポジトリが存在しないため、missing_count が 2 になるはず
    assert_eq!(
        status_result.missing_count, 2,
        "Both repositories should be missing"
    );
}
...
async fn test_end_to_end_workflow() {
    ...
    if workspace.is_initialized() {
        ...
        assert!(
            status_result.total_count() > 0,
            "Should have repositories to check"
        );
    }
    ...
    assert!(
        workspace.manifest_file_path().exists(),
        "manifest.yml should exist"
    );
}
...
async fn test_error_handling_in_workflow() {
    ...
    assert!(
        result.is_err(),
        "Status check should fail for uninitialized workspace"
    );
