...
use tsrc::application::services::manifest_service::{ManifestService, ProcessedManifest, ManifestServiceError};
...
impl MockManifestService {
    ...
    pub async fn parse_from_file(&self, path: &Path) -> Result<ProcessedManifest, ManifestServiceError> {
        ...
        if let Some(manifest) = manifests.get(&path_str) {
            ...
            Err(ManifestServiceError::ParseError(format!("Manifest not found: {}", path_str)))
        }
    }
    
    /// Mock parse_from_url method
    pub async fn parse_from_url(&self, url: &str) -> Result<ProcessedManifest, ManifestServiceError> {
    ...
    pub fn validate_manifest(&self, manifest: &Manifest) -> Result<(), ManifestServiceError> {
        self.record_call("validate_manifest", &format!("{} repos", manifest.repos.len()));
        
        if self.should_return_error() {
            return Err(ManifestServiceError::ValidationError("Mock validation error".to_string()));
        }
        ...
        for repo in &manifest.repos {
            if !destinations.insert(&repo.dest) {
                return Err(ManifestServiceError::ValidationError(
                    format!("Duplicate destination: {}", repo.dest)
                ));
            }
        }
...
impl MockCommandExecutor {
    ...
    pub fn set_command_result(&self, command: &str, exit_code: i32, stdout: &str, stderr: &str) {
        ...
        results.insert(command.to_string(), (exit_code, stdout.to_string(), stderr.to_string()));
    }
