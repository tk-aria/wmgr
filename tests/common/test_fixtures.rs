...
use tsrc::domain::{
    entities::{
        manifest::{Manifest, ManifestRepo, Group},
    ...
    pub fn complex() -> Manifest {
        let repos = vec![
            ManifestRepo::new("https://github.com/example/backend-api.git", "backend/api")
                .with_branch("main"),
            ManifestRepo::new("https://github.com/example/frontend-web.git", "frontend/web")
                .with_branch("develop"),
            ManifestRepo::new("https://github.com/example/build-tools.git", "tools/scripts")
                .with_branch("main"),
        ...
        groups.insert(
            "backend".to_string(),
            Group::new(vec!["backend/api".to_string()])
                .with_description("Backend services"),
        );
        groups.insert(
            "frontend".to_string(),
            Group::new(vec!["frontend/web".to_string()])
                .with_description("Frontend applications"),
        );
        groups.insert(
            "core".to_string(),
        ...
        groups.insert(
            "tools".to_string(),
            Group::new(vec!["tools/scripts".to_string()])
                .with_description("Development tools"),
        );
        groups.insert(
            "docs".to_string(),
            Group::new(vec!["docs/wiki".to_string()])
...
impl RepositoryFixture {
    ...
    pub fn basic(dest: &str) -> Repository {
        let remotes = vec![
            Remote::new("origin", &format!("https://github.com/example/{}.git", dest)),
        ];
    ...
    pub fn with_multiple_remotes(dest: &str) -> Repository {
        let remotes = vec![
            Remote::new("origin", &format!("https://github.com/example/{}.git", dest)),
            Remote::new("upstream", &format!("https://github.com/upstream/{}.git", dest)),
...
impl ValueObjectFixture {
    ...
    pub fn git_urls() -> Vec<(GitUrl, &'static str)> {
        vec![
            (GitUrl::new("https://github.com/example/repo.git").unwrap(), "HTTPS URL"),
            (GitUrl::new("git@github.com:example/repo.git").unwrap(), "SSH URL"),
            (GitUrl::new("https://gitlab.com/example/repo.git").unwrap(), "GitLab HTTPS"),
            (GitUrl::new("git@gitlab.com:example/repo.git").unwrap(), "GitLab SSH"),
        ]
    }
    ...
    pub fn file_paths() -> Vec<(FilePath, &'static str)> {
        vec![
            (FilePath::new_relative("src/main.rs").unwrap(), "Relative file"),
            (FilePath::new_relative("docs/README.md").unwrap(), "Relative nested file"),
        ]
    }
    ...
    pub fn branch_names() -> Vec<(BranchName, &'static str)> {
        vec![
            ...
            (BranchName::new("feature/user-auth").unwrap(), "Feature branch"),
            (BranchName::new("hotfix/security-patch").unwrap(), "Hotfix branch"),
...
mod tests {
    ...
    fn test_filesystem_fixture_git_structure() {
        ...
        assert!(repo_path.join(".git").join("refs").join("heads").join("main").exists());
    }
